...
leaderboard.15[, wod    := factor(stage, levels=c(1,1.1,2,3), labels = c("15.1", "15.1A","15.2","15.3"), ordered = TRUE)]
...
ggplot(leaderboard.15[wod=="15.3" & scaled =="Rx" & !is.na(score),]) +
    ...
    labs(x="15.3 Score",y="# of Athletes", title="Crossfit Open 15.3 Scores (Rx)") +
...
ggsave(filename="~/Desktop/crossfit_15.3rx_hist_gender.png",width=10, height=6)

percentiles.rx <- melt(leaderboard.15[wod=="15.3" & scaled =="Rx" & !is.na(score), 
...
ggplot(percentiles.rx) + 
  ...
  scale_x_continuous(limits = c(0,629),
                     breaks = breaks.rx[,reps])+
  labs(x="15.3 Score (Rx)",y="Percentile", title="Crossfit Open 15.3 Percentile and Scores (Rx)") +
  theme_bw() + 
  theme(legend.position = "none", panel.grid.minor.x = element_blank(), axis.text.x = element_text(angle = 90, hjust = 1))
ggsave(filename="~/Desktop/crossfit_15.3rx_percentile_gender.png",width=10, height=6)

####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#### 15.3 Scaled Histogram ####
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
ggplot(leaderboard.15[wod=="15.3" & scaled =="Scaled" & !is.na(score),]) +
  geom_rect(data=rounds.sc, aes(xmin=x_min, xmax=x_max+1), 
            ymin=0, ymax=10000, alpha=0.2, fill="grey60") +
  geom_vline(data=breaks.sc, aes(xintercept = reps), color="darkgrey", alpha=.5) + 
  geom_histogram(aes(x=score, fill=gender), binwidth=1)+
  scale_x_continuous(limits = c(0,1052),
                     breaks = breaks.sc[,reps])+
  labs(x="15.3 Score",y="# of Athletes", title="Crossfit Open 15.3 Scores (Scaled)") +
  facet_grid(gender~.,scale="free_y") +
  theme_bw(base_size=14) +
  theme(legend.position = "none", panel.grid.minor.x = element_blank())
ggsave(filename="~/Desktop/crossfit_15.3sc_hist_gender.png",width=10, height=6)

percentiles.sc <- melt(leaderboard.15[wod=="15.3" & scaled =="Scaled" & !is.na(score), 
                                      as.list(quantile(score,
                                                       seq(.01,.99,.01))), 
                                      by=gender],
                       id.vars = "gender", variable.name="percentile", value.name="score_cutoff")
...
ggplot(percentiles.sc) + 
  ...
  scale_x_continuous(limits = c(0,1052),
                     breaks = breaks.sc[,reps])+
  labs(x="15.3 Score (Scaled)",y="Percentile", title="Crossfit Open 15.3 Percentile and Scores (Scaled)") +
  theme_bw() + 
  theme(legend.position = "none", panel.grid.minor.x = element_blank())
ggsave(filename="~/Desktop/crossfit_15.3sc_percentile_gender.png",width=10, height=6)

####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#### 15.3 Performance by Profile Stats: Age, Height, Weight ####
####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
### Age
scores.age <- leaderboard.15[wod=="15.3" & !is.na(score),
...
ggplot(scores.age[age >= 17 & age <=55 & !is.na(age) & n_athletes > 50],
  ...
  labs(x="Age", y="Avg Percentile", size="# Athletes", fill="Gender",
       title="Crossfit Open 15.3 Percentile by Age") + 
  theme_bw(base_size = 14) + 
  facet_grid(.~scaled)
ggsave(filename="~/Desktop/crossfit_15.3_age.png",width=10, height=6)


### Height
scores.height <- leaderboard.15[wod=="15.3" & !is.na(score) & 
...
ggplot(scores.height[n_athletes > 50],
  ...
  labs(x="Height (ft, in)", y="Avg Percentile", size="# Athletes", color="Gender",
       title="Crossfit Open 15.3 Percentile by Height") + 
  theme_bw(base_size = 14) + 
  facet_grid(.~scaled)
ggsave(filename="~/Desktop/crossfit_15.3_height.png",width=10, height=6)

### Weight
scores.weight <- leaderboard.15[wod=="15.3" & !is.na(score),
                                list(perc_avg = mean(percentile),
                                     n_athletes = .N),
...
ggplot(scores.weight[weight < 400 & weight >= 80 & n_athletes > 50],
  ...
  labs(x="Weight (10 lb bins)", y="Avg Percentile", size="# Athletes", color="Gender",
       title="Crossfit Open 15.3 Percentile by Weight") + 
  theme_bw(base_size = 14) + 
  facet_grid(.~scaled)
ggsave(filename="~/Desktop/crossfit_15.3_weight.png",width=10, height=6)
